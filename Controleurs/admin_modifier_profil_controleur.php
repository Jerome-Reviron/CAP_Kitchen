                                    ...
                                    if($Password == $Password_retype){ // si les deux mdp saisis sont bons
                                        if (preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$^+,=!*()@%&]).{8,}$/', $Password)) {
                                            ...
                                            if ($updateAdminResult ) {
                                                ...
                                                header("Location: index.php?uc=admin_modifier_profil");
                                            }
                                        }else{ 
                                            header('Location: index.php?uc=admin_modifier_profil&reg_err=password1'); 
                                        }
                                    }else{ 
                                        header('Location: index.php?uc=admin_modifier_profil&reg_err=password2'); 
                                        exit();
                                    }
                                }else{ 
                                    header('Location: index.php?uc=admin_modifier_profil&reg_err=adresse_length'); 
                                    exit();
                                }
                            }else{ 
                                header('Location: index.php?uc=admin_modifier_profil&reg_err=email');
                                exit();
                            }
                        }else{ 
                            header('Location: index.php?uc=admin_modifier_profil&reg_err=email_length'); 
                            exit();
                        }
                    }else{ 
                        header('Location: index.php?uc=admin_modifier_profil&reg_err=telephone_length'); 
                        exit();
                    }
                }else{ 
                    header('Location: index.php?uc=admin_modifier_profil&reg_err=pseudo_length'); 
                    exit();
                }
            } else {
                // Génère et stocke un nouveau jeton CSRF dans la session
                $csrf_token = $Securiter->generateCsrfToken();
                include './Vues/admin_modifier_profil_vue.php';
            }
        }else{
